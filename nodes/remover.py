...
class LamaModelManager:
    """
    ✅ 單例模型管理器 (Singleton Model Manager)
    - 全局管理 BigLama 模型實例，確保模型只載入一次。
    - 包含預熱機制，首次使用時會執行一次 dummy 推理，以完全初始化 TensorRT 引擎，
      讓後續的真實任務能以最高效能運行。
    """
    _model = None
    ...
    def get_model(cls):
        ...
        if cls._model is None:
            print("Initializing BigLama model for the first time...")
            cls._model = model.BigLama()

            # --- 預熱機制 (Warm-up Mechanism) ---
            print("Warming up the TRT engine... (This might take a moment on first run)")
            try:
                # 創建與模型相同設備的 dummy 張量
                device = cls._model.device
                dummy_image = torch.zeros((1, 3, 512, 512), device=device)
                dummy_mask = torch.zeros((1, 1, 512, 512), device=device)

                # 執行一次推理以觸發所有初始化和優化
                with torch.no_grad():
                    _ = cls._model(dummy_image, dummy_mask)

                print("✅ BigLama model loaded, warmed up, and ready for lightning-fast inference.")
...
class LamaRemover:
    ...
    def lama_remover(self, images, masks, mask_threshold, gaussblur_radius, invert_mask):
        # ✅ 優化：從單例管理器獲取模型，而非每次重新創建
        mylama = LamaModelManager.get_model()
        results = []
...
NODE_DISPLAY_NAME_MAPPINGS = {"LamaRemover": "Big Lama Remover (TRT+CUDA)", "LamaRemoverIMG": "Big Lama Remover (IMG, TRT+CUDA)"}